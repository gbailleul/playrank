...
const CricketDartBoard: React.FC<CricketDartBoardProps> = ({
  ...
  const getTargetStatus = (target: CricketTarget) => {
    ...
    // S'assurer que le score existe pour cette cible
  ...
  const createSection = (index: number, score: number) => {
    ...
    const getBaseColor = (pathIndex: number) => {
        ...
        {paths.map((path, pathIndex) => (
          <path
            ...
            className={`cursor-pointer stroke-[#a0a0a0] stroke-1 transition-colors hover:brightness-150 hover:opacity-80 ${
              isHit(pathIndex) ? 'fill-[var(--neon-primary)] opacity-70' : getBaseColor(pathIndex)
    ...
    <div className="relative inline-block">
      ...
      {dartHits.length === 3 && (
        <div className="mt-4 text-center">
          <button
            onClick={() => {
              // Envoyer les scores dans l'ordre des lancers
              for (const hit of dartHits) {
                if (!hit.isMiss) {
                  onScoreClick(hit.target, hit.multiplier);
                }
              }
              // Réinitialiser les fléchettes et passer au joueur suivant
              setDartHits([]);
              onTurnComplete();
            }}
